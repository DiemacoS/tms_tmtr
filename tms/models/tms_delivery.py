...
class TmsDelivery(models.Model):
    ...
    def add_orders_from_delivery(self):
        ...
        for record in records:
            dublicate_order = self.env['tms.order'].search_count([
            ...
            delivery_rows = self.env['tms.delivery.row'].search([('delivery_id', '=', record.id)])
    ...
    def _get_unique_order_number(self, order_num):
        ...
        for suffix in range(1, existing_order_numbers + 1):
            new_order_number = f"{order_num}_{suffix}"
            if not self.env['tms.order'].search([('order_num', '=', new_order_number)]):
