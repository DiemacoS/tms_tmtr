...
class TmsOrder(models.Model):
            ...
            record_points = [
                {
                 'id': point.id,
                 'company_name': (
                     point.route_point_id.res_partner_id.company_name or point.route_point_id.res_partner_id.name
                 ),
                 ...
                 'returned_client': (
                     point.returned_client.astimezone(current_timezone) if point.returned_client else False
                 ),
                 ...
                 'order_row_type': point.order_row_type if point.order_row_type else (
                     'return' if 'Возврат' in point.impl_num else False
                 ),
                 'cancel_delivery': (
                     point.cancel_delivery.astimezone(current_timezone) if point.cancel_delivery else False
                 ), #Время отмены заказа
    ...
    def name_get(self):
        return [(
            record.id, '{num}, {number}, {date} ({route})'.format(
                num=record.order_num,
                route=record.route_id.name,
                number=record.order_num,
...
class TMSAttendance(models.Model):
    ...
    def getDriverAttendance(self, driver_ids):
        return [{
            ...
            'check_id': r.check_in
        } for r in self.search([('employee_id','in',driver_ids),('check_out','=',False)])]
