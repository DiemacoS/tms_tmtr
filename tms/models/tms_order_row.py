...
class TmsOrderRow(models.Model):
    ...
    def saveDatesTmsOrderRow(self, dataTmsOrderRow, user_id, browse_id):
        for dataAction in dataTmsOrderRow:
            ...
            if 'switch_status_row' in dataAction and dataAction['action'] == 'clear_date_row':
                ...
                for key, value in vals.items():
                    if key in record and value != record[key]:
                        changed_fields.append(f"{key} {record[key]} UTC+0")

                if changed_fields:
                    record.order_id.message_post(
                        body=record.impl_num + ' сброс старых данных: ' + ', '.join(changed_fields)
                    )
