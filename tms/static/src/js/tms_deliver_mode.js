odoo.define('tms.deliver_mode', function (require) {
    ...
    let TmsDeliverMode = AbstractAction.extend({
        events: {
        ...
        getAllDataFromCache: function () {
            return new Promise((resolve, reject) => {
                let transaction = this.database.idb.transaction(this.database.idb_stores.tms_state_cache, 'readonly');
                let objectStore = transaction.objectStore(this.database.idb_stores.tms_state_cache);
                let getRequest = objectStore.getAll();
        ...
        showInterfaceActual: function() {
            this.$el.html(QWeb.render('TmsBase',
                {'context': this.tmsContext, 'widget': this.widget, 'action': this.action}));
        },

        display_message: function(viewTypeMessage) {
            if (viewTypeMessage === 'confirm') {
                this.$(".o_tms_modal_confirm").html(QWeb.render('TmsModalConfirm',
                    {'context': this.tmsContext, 'widget': this.widget, 'action': this.action}));
            }
            if (viewTypeMessage === 'feedback') {
                this.$(".o_tms_modal_feedback").html(QWeb.render('TmsModalFeedBack',
                    {'context': this.tmsContext, 'widget': this.widget, 'action': this.action}));
            }
            if (viewTypeMessage === 'switchStatusOrder') {
                this.$(".o_tms_modal_switch_s_order").html(QWeb.render('TmsModalSwitchStatusOrder',
                    {'context': this.tmsContext, 'widget': this.widget, 'action': this.action}));
            }
        },
        ...
        showConcreteRoutePoint: async function(ev) {
            ...
            this.routes.forEach( (route) => {
                let foundPoint = route.points.find((point) => {
        ...
        saveStateInIndexeddb: function(){
            let transaction = this.database.idb.transaction(this.database.idb_stores.tms_state_cache, 'readwrite');
            let objectStore = transaction.objectStore(this.database.idb_stores.tms_state_cache);

            let clearRequest = objectStore.clear();

            clearRequest.onsuccess = function (event) {
                let putRequest = objectStore.put(this.tmsContext);
        ...
        saveRoutesInCache: function(routes){
            let transaction = this.database.idb.transaction(this.database.idb_stores.tms_routes_cache, 'readwrite');
            let objectStore = transaction.objectStore(this.database.idb_stores.tms_routes_cache);

            let clearRequest = objectStore.clear();

            clearRequest.onsuccess = function (event) {
                let putRequest = objectStore.put(routes);
                putRequest.onsuccess = function (event) {
        ...
        saveBrowseUidUserInCache: function() {
            let transaction = this.database.idb.transaction(this.database.idb_stores.tms_browser_uid, 'readwrite');
            let objectStore = transaction.objectStore(this.database.idb_stores.tms_browser_uid);

            let clearRequest = objectStore.clear();

            clearRequest.onsuccess = function (event) {
                console.log(this.browse)
                let putRequest = objectStore.put(this.browse);
                putRequest.onsuccess = function (event) {
